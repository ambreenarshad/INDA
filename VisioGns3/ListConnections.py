...
def main(pages_xml, masters_xml, output_json):
    ...
    # Group connections by their FromSheet value
    for conn in connections:
        from_sheet = conn["from_sheet"]
        if from_sheet not in connection_groups:
            connection_groups[from_sheet] = []
        connection_groups[from_sheet].append(conn)

    # Process grouped connections
    for from_sheet, group in connection_groups.items():
        if len(group) == 2:  # Ensure there are exactly two connections in the group
            conn1, conn2 = group
            ...
            if start_shape and end_shape:
                ...
                start_with_id = f"{start_name}{start_sheet}".replace(" ", "")
                end_with_id = f"{end_name}{end_sheet}".replace(" ", "")
